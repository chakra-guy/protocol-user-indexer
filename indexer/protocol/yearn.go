...
func NewYearn(store *store.Store, ethclient *ethclient.Client) *YearnIndexer {
	return &YearnIndexer{
		id:                 2,
		store:              store,
		ethclient:          ethclient,
...
var SPEC_CONDITION_EVENT_NAME = "Approval"
var SPEC_USER_EVENT_ARG = "owner"

func (indexer *YearnIndexer) Index() error {
	start := time.Now()
	...
	for _, log := range logs {
		if log.Topics[0] == contractABI.Events[SPEC_CONDITION_EVENT_NAME].ID {
			event := make(map[string]interface{})

			err := contractABI.UnpackIntoMap(event, SPEC_CONDITION_EVENT_NAME, log.Data)
			if err != nil {
				return err
			}

			var i = 1 // topic_name_0 is the event signature so we start from 1
			for _, input := range contractABI.Events[SPEC_CONDITION_EVENT_NAME].Inputs {
			...
			fmt.Printf("\n 🟢🟢 EVENT: %s\n\n", eventjson)
			fmt.Printf("\n 🟡🟡 USER: %+v\n\n", event[SPEC_USER_EVENT_ARG])

			user, ok := eventSimple[SPEC_USER_EVENT_ARG].(string)
			if ok {
				fmt.Printf("\n 🔵🔵 USER: %s\n\n", user)
